...
public final class RdbParser implements AutoCloseable {
	...
	/**
	 * <p>
	 * Returns the number of bytes parsed from the underlying file or stream by
	 * successive calls of the {@link #readNext} method.
	 *
	 * <p>
	 * As RdbParser uses a buffer internally, the returned value will be slightly
	 * smaller than the total number of bytes buffered from the underlying file or
	 * stream.
	 *
	...
	private long readLength() throws IOException {
		...
		if (flag == 0) { // 00|XXXXXX: len is the last 6 bits of this byte.
			...
			byte[] bs = readBytes(4);
			return ((long) bs[0] & 0xff) << 24 | ((long) bs[1] & 0xff) << 16 | ((long) bs[2] & 0xff) << 8
					| ((long) bs[3] & 0xff) << 0;
			...
			return ((long) bs[0] & 0xff) << 56 | ((long) bs[1] & 0xff) << 48 | ((long) bs[2] & 0xff) << 40
					...
					| ((long) bs[6] & 0xff) << 8 | ((long) bs[7] & 0xff) << 0;
		} else {
	...
	private byte[] readStringEncoded() throws IOException {
		...
		case 2: // read next four bytes as unsigned big-endian
			byte[] bs = readBytes(4);
			len = ((int) bs[0] & 0xff) << 24 | ((int) bs[1] & 0xff) << 16 | ((int) bs[2] & 0xff) << 8
					| ((int) bs[3] & 0xff) << 0;
			if (len < 0) {
				throw new IllegalStateException(
						"Strings longer than " + Integer.MAX_VALUE + "bytes are not supported.");
			}
			return readBytes(len);
		case 3:
	...
	private byte[] readInteger16Bits() throws IOException {
		long val = ((long) readByte() & 0xff) << 0 | (long) readSignedByte() << 8; // Don't apply 0xff mask to preserve
																					// sign.
	...
	private byte[] readInteger32Bits() throws IOException {
		...
		long val = (long) bs[3] << 24 // Don't apply 0xff mask to preserve sign.
				| ((long) bs[2] & 0xff) << 16 | ((long) bs[1] & 0xff) << 8 | ((long) bs[0] & 0xff) << 0;
		return String.valueOf(val).getBytes(ASCII);
	}
	...
	private void readList() throws IOException {
		...
		if (len > Integer.MAX_VALUE) {
			throw new IllegalArgumentException(
					"Lists with more than " + Integer.MAX_VALUE + " elements are not supported.");
		}
		int size = (int) len;
	...
	private void readSet() throws IOException {
		...
		if (len > Integer.MAX_VALUE) {
			throw new IllegalArgumentException(
					"Sets with more than " + Integer.MAX_VALUE + " elements are not supported.");
		}
		int size = (int) len;
	...
	private void readSortedSet() throws IOException {
		...
		if (len > (Integer.MAX_VALUE / 2)) {
			throw new IllegalArgumentException(
					"SortedSets with more than " + (Integer.MAX_VALUE / 2) + " elements are not supported.");
		}
		int size = (int) len;
	...
	private void readSortedSet2() throws IOException {
		...
		for (int i = 0; i < size; ++i) {
			...
			valueScoresPairs
					.add(String.valueOf(Double.valueOf(parseSortedSet2Score(readBytes(8))).intValue()).getBytes());
		}
	...
	private void readHash() throws IOException {
		...
		if (len > (Integer.MAX_VALUE / 2)) {
			throw new IllegalArgumentException(
					"Hashes with more than " + (Integer.MAX_VALUE / 2) + " elements are not supported.");
		}
		int size = (int) len;
	...
	private void readQuickList() throws IOException {
		...
		if (len > Integer.MAX_VALUE) {
			throw new IllegalArgumentException(
					"Quicklists with more than " + Integer.MAX_VALUE + " nested Ziplists are not supported.");
		}
		int size = (int) len;
